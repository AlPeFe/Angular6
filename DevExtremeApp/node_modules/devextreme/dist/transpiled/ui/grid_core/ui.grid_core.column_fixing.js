    ...
    _createCell: function _createCell(options) {
        ...
        if (that._isFixedTableRendering) {
            if (columnCommand === "transparent") {
                $cell.addClass(POINTER_EVENTS_NONE_CLASS).toggleClass(FIRST_CELL_CLASS, transparentColumnIndex === 0 || prevFixedColumn && prevFixedColumn.command === "expand").toggleClass(LAST_CELL_CLASS, fixedColumns.length && transparentColumnIndex === fixedColumns.length - 1).html(rowType !== "freeSpace" ? "&nbsp;" : "");
            }
    ...
    getContextMenuItems: function getContextMenuItems(options) {
        ...
        if (options.row && options.row.rowType === "header") {
            if (column && column.allowFixing) {
                ...
                items = items || [];
                items.push({
                    text: columnFixingOptions.texts.fix, beginGroup: true, items: [{ text: columnFixingOptions.texts.leftPosition, value: "left", disabled: column.fixed && (!column.fixedPosition || column.fixedPosition === "left"), onItemClick: onItemClick }, { text: columnFixingOptions.texts.rightPosition, value: "right", disabled: column.fixed && column.fixedPosition === "right", onItemClick: onItemClick }]
...
var RowsViewFixedColumnsExtender = extend({}, baseFixedColumns, {
    ...
    _updateFixedTablePosition: function _updateFixedTablePosition(scrollTop) {
