...
var getValueAxesPerPanes = function getValueAxesPerPanes(valueAxes) {
    var result = {};
...
var convertAxisInfo = function convertAxisInfo(axisInfo, converter) {
...
var populateAxesInfo = function populateAxesInfo(axes) {
    return axes.reduce(function (result, axis) {
        ...
        if (majorTicks && majorTicks.length > 0 && typeUtils.isNumeric(majorTicks[0]) && options.type !== "discrete" && !businessRange.stubData && !(businessRange.breaks && businessRange.breaks.length)) {
...
var updateTickValues = function updateTickValues(axesInfo) {
    var maxTicksCount = 0;
...
var getAxisRange = function getAxisRange(axisInfo) {
    return axisInfo.maxValue - axisInfo.minValue || 1; // T153054
};

var getMainAxisInfo = function getMainAxisInfo(axesInfo) {
...
var correctMinMaxValues = function correctMinMaxValues(axesInfo) {
...
var calculatePaddings = function calculatePaddings(axesInfo) {
    var minPadding,
        maxPadding,
        startPadding = 0,
        endPadding = 0;
...
var correctMinMaxValuesByPaddings = function correctMinMaxValuesByPaddings(axesInfo, paddings) {
...
var updateTickValuesIfSynchronizedValueUsed = function updateTickValuesIfSynchronizedValueUsed(axesInfo) {
...
var applyMinMaxValues = function applyMinMaxValues(axesInfo) {
    _each(axesInfo, function (_, info) {
        var axis = info.axis,
...
var correctAfterSynchronize = function correctAfterSynchronize(axesInfo) {
    var invalidAxisInfo = [],
        correctValue,
        validAxisInfo;
