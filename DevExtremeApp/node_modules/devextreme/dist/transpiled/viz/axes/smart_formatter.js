...
function createFormat(type) {
    ...
    return { format: { type: type, formatter: formatter } };
}
...
function smartFormatter(tick, options) {
    ...
    if (!(0, _type.isDefined)(format) && (0, _type.isDefined)(tickInterval) && options.type !== "discrete" && tick && (options.logarithmBase === 10 || !isLogarithmic)) {
    ...
    return (0, _format3.default)(tick, { format: format, precision: options.labelOptions.precision });
}
